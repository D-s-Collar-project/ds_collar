...
register_self() {
    // Announce full registration. The kernel is the SOLE writer of reg.<ctx>
    // (and acl.policycontext) — see collar_kernel rev 6. Status is view-only
    // with an empty per-button policy, so no policy field is declared.
