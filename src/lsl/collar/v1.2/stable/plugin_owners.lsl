...
- v1.2 rev 10: optimization pass (script at 92.6% Mono, bytecode-bound). Extracted start_scan() — the avatar-scan setup was inlined at 5 sites (menu add-owner/transfer/add-trustee + chat add-owner/add-trustee); 5 callers amortize LSL's ~128B/function overhead, net -360B measured. Fixed set_confirm: it emitted ui.menu.return AFTER cleanup() (which nulls CurrentUser), so the return carried NULL_KEY — now emits before cleanup, matching the Back path. Deliberately did NOT extract finish_scan/return_to_root/notify helpers: at 2 callers each the function overhead exceeds the dedup saving (would grow bytecode).
- v1.2 rev 9: menu-service migration stage 3 (final) — confirms now render via the service's MODAL mode (mode "modal") instead of a raw [Yes,No] ui.dialog.open. The service forces No (the safe choice) to slot 0 and still returns confirm/cancel, so every confirm branch is unchanged — render-only swap. All owners dialogs now flow through kmod_menu (pager main / UL scans + honorific / OL remove-trustee / modal confirms); DIALOG_BUS retained for response + close routing only.
- v1.2 rev 8: menu-service migration stage 2 — the pickers. Owner/transfer/trustee scans render as UL (mode "unordered", {label:name, context:uuid} items; the click returns the UUID, so long/colliding display names can't misselect); show_remove_trustee + honorific pickers render via the service too (remove = OL pick:<idx> into TrusteeKeys; honorific = UL flat, click returns the honorific). Replaced open_numbered_dialog with render_list_picker; added CurrentPage + << >> paging (picker_count/redraw_picker, LIST_PAGE_SIZE 9 must match kmod_menu), dropping the 11-item cap. Nav arrives as cmd=""/label=arrow (no cmd-fallback here) so paging matches on label; content picks match on cmd. Confirms still use the legacy Yes/No show_confirm — modal migration is stage 3.
- v1.2 rev 7: menu-service migration stage 1 — show_main now renders via the pager (ui.menu.render, has_nav=1) instead of a raw ui.dialog.open; dropped the hand-rolled Back button (the service supplies the << >> Back nav) and added an inert-<< >> redraw fallback in the main handler block. Sub-flows (scans/confirms) still use the legacy numbered_list path; migrated in later stages.
