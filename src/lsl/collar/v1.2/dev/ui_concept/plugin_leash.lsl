...
CHANGES:
- v1.2 rev 8 (sandbox): ABSORBED plugin_leash_target — the former hidden picker sub-plugin is now in-line. The main menu / chat verbs call startPicker() directly instead of delegating over ui.menu.start, so the whole delegation seam is gone (delegateTo + returnToParent + the ui.menu.start re-entry/subpath redispatch). Picker renders via OL mode, offer-reception via modal mode. The two scripts' action senders collapse onto one sendAction(action, extra, recipient). One MenuContext spans the menu (main/settings/texture/length) + picker (pass/offer/coffle/post) states; the dialog-response router dispatches picker-vs-menu by MenuContext and the offer by its own session. sensor()/no_sensor() (Post object scan) move here. Merged footprint ~56.4 KB / 86% of the Mono ceiling; retires plugin_leash_target (was leash 42.3 KB + target 25.2 KB across two scripts). Nav-row consistency: has_nav 0→1 on showMenu (full << >> Back row on every menu, per the project convention), length's << >> repurposed as -1m/+1m, catch-all redraws for the inert << >> on the other menus.
- v1.2 rev 7 (sandbox): menu-service migration + bytecode dedup. All four menus (main/settings/texture/length) now render via the pager (showMenu → ui.menu.render, DIALOG_BUS→UI_BUS); callers pass CONTENT buttons only and the service supplies Back + layout, so reorder_item_buttons is deleted outright. Length's << / >> (-1m/+1m fine-tune) ride as content buttons, not pager nav. Response handler maps the service's plain Back → "back" so the per-menu back branches are unchanged. The four near-identical senders (sendLeashAction / *WithTarget / sendSetLength / inline set_texture) collapse onto one sendAction(action, extra) builder — removes 3x the llMessageLinked/JSON boilerplate. PLUGIN_CATEGORY/MASK hoisted to the identity block (render_menu reads category). No behavior change beyond Back/layout now owned by the service.
