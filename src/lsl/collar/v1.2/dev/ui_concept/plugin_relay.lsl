...
CHANGES:
- v1.2 rev 9 (sandbox): nav-row consistency — has_nav 0→1 on all three menus so the << >> Back row matches the rest of the UI (was a lone Back); the handler's existing catch-all redraws the inert << >>.
- v1.2 rev 8 (sandbox): menu-service migration. show_main_menu / show_mode_menu / render_object_list now render via the pager (ui.menu.render, has_nav=0; the "Bound by" source list is an info pager — body + Back, no content buttons). message→body, the local "Back" dropped from each list (the service supplies it), sends moved DIALOG_BUS→UI_BUS. Buttons stay plain label strings — render_menu treats the list opaquely and the handler already routes by button label — so handle_button_click is unchanged. Mode/hardcore/safeword/source logic untouched. (Back still exits to root from every relay screen, as before.)
