...
REVISION: 9
CHANGES:
- v1.2 rev 9 (sandbox): nav-row consistency — has_nav 0→1 on all four menus so the << >> Back row matches the rest of the UI; added catch-all redraws in main/owner/trustee for the now-inert << >> (toggle menus already self-redraw to the parent).
- v1.2 rev 8 (sandbox): menu-service migration. show_main / show_owner_menu / show_trustee_menu / show_toggle now render via the pager (ui.menu.render, has_nav=0; the service supplies Back), sends moved DIALOG_BUS→UI_BUS, local "Back" button dropped from each list. Content buttons keep their {label,context} so handle_button's ctx-routing is unchanged; the response handler maps the service's plain Back (button="Back", empty ctx) → "back" so the context-aware up-navigation (toggle→role menu→main→exit) is preserved. Exception logic untouched.
