...
- v1.10 rev 2: Replace sequential Yes/No walk with a one-stop toggle
  ...
  the project dialog convention. RLV Subsystem entry drills down to a
  secondary dialog; main row's [X]/[ ] mark is derived from "any RLV
  plugin selected". Page size = 8 (one Install action at slot 5),
  wrap-around pagination. Default state: all toggles off — wearer
  explicitly opts into what they want.
- v1.10 rev 1: Initial implementation — sequential Yes/No walk.
--------------------*/
...
list bespoke_subsystems() {
    return [
        ...
        "Leashing",       "kmod_leash_engine,kmod_particles,kmod_leash_proto,plugin_leash,plugin_leash_avatar,plugin_leash_object,leash_holder",
        "Locking",        "plugin_lock",
